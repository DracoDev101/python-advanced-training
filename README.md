...
> 面向已有 Python 基础、希望系统掌握 Django 生产级后端、异步任务、ASGI/WSGI 部署、并发模型与线上排障能力的学习者。重点不是语法入门，而是 Python 运行时机制、Django 工程化、Celery 任务系统、Daphne/Gunicorn 部署模型、Redis/Redis Stream、MySQL、MongoDB、Kafka、greenlet/gevent/eventlet 与 monkey patch 边界，以及生产环境可靠性设计。
...
- 在 Django 系统中合理使用 Redis、Redis Stream、MySQL、MongoDB、Kafka，并能解释一致性、消费语义和排障证据。
...
建议周期：**12–15 周**，共 30 lessons。每周 2 lessons + 1 次实验或项目增量。
...
3. API、认证、缓存、文件与后台管理
4. Redis、Redis Stream、MySQL、MongoDB 与查询系统
5. Celery、Kafka 与可靠异步架构
6. WSGI/ASGI、Daphne、Gunicorn 与并发模型
7. greenlet/gevent/eventlet 与 monkey patch 边界
8. Observability、性能诊断与生产排障
9. 综合项目：Production Order Workflow
